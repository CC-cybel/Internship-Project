...
请撰写详细的中文分析报告，并在报告末尾附上Markdown表格总结关键发现。"""
        ...
        needs_prefetch = (
            'dashscope' in llm.__class__.__name__.lower()
            or 'deepseek' in llm.__class__.__name__.lower()
            or 'zhipu' in llm.__class__.__name__.lower()
            or 'dashscope' in model_info_lower
            or 'qwen' in model_info_lower
            or 'deepseek' in model_info_lower
            or 'zhipu' in model_info_lower
            or 'glm' in model_info_lower
        )
        if needs_prefetch:
