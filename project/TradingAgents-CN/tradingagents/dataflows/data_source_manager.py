...
def _run_async_safely(coro):
    """Run an async provider call from sync code, even inside FastAPI active event loop."""
    try:
...
class DataSourceManager:
        ...
        try:
            ...
            if is_success:
                ...
                if fallback_result and "❌" not in fallback_result and "错误" not in fallback_result:
                    logger.info(f"✅ [数据来源: 备用数据源] 降级成功获取数据: {symbol} ({fallback_source})")
        ...
        try:
            ...
            if cached_data is not None and not cached_data.empty:
                ...
                if provider:
                    ...
                    stock_info = _run_async_safely(provider.get_stock_basic_info(symbol))
            ...
            data = _run_async_safely(provider.get_historical_data(symbol, start_date, end_date))
    ...
    def _get_akshare_data(self, symbol: str, start_date: str, end_date: str, period: str = "daily") -> str:
        ...
        try:
            ...
            data = _run_async_safely(provider.get_historical_data(symbol, start_date, end_date, period))
