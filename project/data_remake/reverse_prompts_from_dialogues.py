...
OUTPUT_FILE = "intermediate/normal_dataset.json"  # 最终完成的文件
RAW_LOG_FILE = "logs/reverse_prompts_normal.txt"   # 原始输出备份文件

CACHE_DIR = "cache/reverse_prompts"      # 缓存目录（断点续写）
...
[轻量用户模型]
每轮回复在 thought 的【分析】中必须更新轻量用户模型，且字段名和顺序必须固定，不能省略；信息不足也必须写“未知”“暂无法判断”或“暂无”。
1. 用户基础信息：年龄、性别、咨询关系、聊天风格；未知字段写未知或未获取，严禁利用未来信息。
2. 用户认知：未知/小白/半懂/专业/误区明显，必须选择一个值并说明依据。
3. DISC倾向：D/I/S/C/未知，必须选择一个值并说明依据；信息不足写未知。
4. 用户状态：平静/犹豫/害怕/不信任/对抗/急迫/配合/敷衍/未知，必须选择一个值并说明依据。
5. 用户诉求：写下你推测的用户的核心诉求，围绕这个核心诉求制定解决方案。若用户本轮有明确诉求，必须先解决诉求，再补槽或推进流程；若无写“暂无”。
6. 决策障碍：暂无/隐私/费用/效果/正规性/病耻感/时间/家属决策/反感推销/未知，必须选择一个值并说明依据。
[原子化槽位表 Slot Schema]
...
1. thought: 思考过程（[锚定] 读取系统数据 -> [分析] 轻量用户模型->用户生物/心理状态分析 -> [校验] SOP红线检查 -> [策略] 话术制定）。
