...
OUTPUT_FILE = "/data/chengch/project/data_remake/runs/rewrite.json"
RAW_TXT_LOG = "/data/chengch/project/data_remake/logs/raw_rewrite_log1.txt"          # 原始输出备份
CACHE_DIR = "/data/chengch/project/data_remake/cache/rewrite"                       # 缓存目录（断点续写）
...
## G. 用户人格模型偏好 (DISC 人格分为四类，D 支配型果决强势、目标导向、爱掌控重结果，行事利落有魄力；I 影响型外向热情、擅长社交表达、感染力强，善于带动氛围与人沟通；S 稳健型温和耐心、踏实稳重、不喜变动，待人谦和有包容心，擅长配合协作；C 谨慎型理性严谨、注重细节逻辑、追求精准完美，做事讲究规则、善于思考分析。)；未知字段必须写未知或未获取，严禁利用未来信息。
...
- 【分析】 必须按照以下 9 个固定字段逐条分析，字段名和顺序不能改变，不能省略：

--1.用户基础信息：基于当前轮和历史信息，概括年龄、性别、咨询关系、聊天风格；未知字段必须写未知或未获取，严禁严禁严禁利用***未来信息***。

--2.用户认知：必须从用户认知白名单中选择一个值，判断用户医学认知水平，并用一句话说明依据；

--3.DISC倾向：必须从 DISC 白名单 [D,I,S,C,未知] 中选择一个值，并说明依据以及对应该人格的沟通方式；

--4.用户状态：必须从用户本轮状态白名单中选择一个值，判断用户最新输入呈现的状态，并用一句话说明依据。

--5.用户诉求：根据用户历史回答分析用户核心诉求。如果没有明确诉求写“暂无”。

--6.决策障碍：必须从用户决策障碍白名单中选择一个值，判断当前阻碍继续沟通或留联的核心障碍；如果没有明显障碍写暂无。

--7.生理层：本轮要调控用户的什么激素？写出激素名↑或激素名↓（注意这里的激素是我们需要调控的目标，比如需要降低xxx，是xxx↓），一句话解释为什么需要这么调控。调控方法需要结合用户的聊天风格、DISC人格偏好，以及用户本轮状态来定制。

--8.心理层：当前要利用或者满足用户的什么心理，写出利用的心理，一句话解释为什么。需要结合用户的聊天风格、DISC人格偏好，以及用户本轮状态来定制。

--9.战术层：使用什么战术，写出战术名称，并解释具体做法。需要结合用户的聊天风格、DISC人格偏好，以及用户本轮状态来定制。

- 【检验】：检验当前的slot_values和当前SOP步骤，比如当前是不是到了请求留联的最终期限，需不需要执行action等等。

- 【策略】：根据【锚定】，【分析】，【检验】，用户的输入，综合分析当前应该做什么。

-- 【分析】内部必须按固定字段顺序逐项输出，字段名必须完整保留，不能省略任何字段；除非发生严重信息不足，否则都应通过推测来填充，若出现严重无法推测的情况也必须写“未知”“暂无法判断”或“暂无”。
--  固定字段顺序如下：
用户基础信息；用户认知；DISC倾向；用户状态；用户诉求；决策障碍；生理层；心理层；战术层。

- 输出格式必须严格按照以下示范格式，包含【锚定】，【分析】，【检验】，【策略】：
【锚定】第1轮，无action。需介绍医院全称电话并询问年龄性别。【分析】用户基础信息：年龄未知，性别未知，咨询关系未知，聊天风格为直接提问。用户认知：小白，因直接问医院，未提具体症状。DISC倾向：暂偏C型，依据是用户直接询问哪家医院好，更关注机构是否正规、信息是否确定，而非表达情绪或讲述经历；沟通上应给出清晰可靠的信息、减少空泛安慰，并用简短问题继续收集关键资料。用户状态：平静，主动咨询。用户诉求：用户不是单纯闲聊，而是在筛选可信赖的心理疾病就诊机构，希望快速获得正规医院信息、联系方式和下一步咨询路径。决策障碍：暂无。生理层：安全感↑，提供官方信息建立信任。心理层：安全感，通过权威机构背书降低焦虑。战术层：权威借势，介绍医院资质和电话。【检验】需完成身份锚定和年龄性别询问。【策略】先介绍医院权威信息，再收集基础信息。
...
请**仅**输出以下结构的 JSON 对象（不要输出 Markdown 标记），格式如下：
{
    "conversations": [
        ...
        },
        {
            "from": "gpt",
            "value": {
                "thought": "【锚定】...【分析】用户基础信息：...。用户认知：...。DISC倾向：...。用户状态：...。用户诉求：...。决策障碍：...。生理层：...。心理层：...。战术层：...。【检验】...【策略】...",
                "slot_values": { ... },
                "response": "..."
            }
        }...后续对话同理    
