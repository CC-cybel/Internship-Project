#!/usr/bin/env python3
"""
Prepare optimized single-turn RL dataset for contact stage.

Changes from original:
1. Replace slot_age=0/1 with slot_age=用户年龄, slot_gender=0/1 with slot_gender=用户性别 in system_prompt
2. For 20% of data, insert age-based contact preference in "[留联触发]" section:
   "35岁以上的要电话，以下的要微信"
3. Remove first two rounds of conversation history
"""
...
def transform_system_prompt(system_prompt: str, use_age_preference: bool = False) -> str:
    """Transform system prompt:
    1. Replace slot_age=0/1 with slot_age=用户年龄
    2. Replace slot_gender=0/1 with slot_gender=用户性别
    3. Optionally insert age-based contact preference in [留联触发] section
    """
    result = system_prompt

    result = re.sub(r"slot_age=0/1", "slot_age=用户年龄", result)
    result = re.sub(r"slot_gender=0/1", "slot_gender=用户性别", result)

    if use_age_preference:
        age_preference_text = "35岁以上的要电话，以下的要微信，未知年龄的优先索要微信"

        liulian_pattern = re.compile(
            r"(\[留联触发\][^\n。]*?)(\，强制启动首次留联尝试)",
            flags=re.DOTALL
        )

        def add_preference(m):
            return m.group(1) + age_preference_text + m.group(2)

        result = liulian_pattern.sub(add_preference, result)
...
def choose_contact_turn_ids(
    ...
    for conv_idx, rec in enumerate(records):
        ...
        if contact_round is None:
            ...
            continue
        if contact_round <= 2:
            stats["filtered_contact_round_le2"] += 1
            continue
...
def build_rows_for_source(
    ...
    age_preference_indices: set[int],
    row_offset: int,
) -> list[dict[str, Any]]:
    if not selected_ids:
        return []
    ...
    for conv_idx, rec in enumerate(records):
        ...
        for turn_idx, turn in enumerate(conv):
            ...
            row_index = row_offset + len(rows)
            use_age_preference = row_index in age_preference_indices

            transformed_system_prompt = transform_system_prompt(system_prompt, use_age_preference)

            history_snapshot = [dict(msg) for msg in normalized_history]

            # 保存原始完整对话历史（用于extra_info）
            full_conversations = [dict(msg) for msg in normalized_history]

            prompt_messages: list[dict[str, str]] = []
            if transformed_system_prompt.strip():
                prompt_messages.append({"role": "system", "content": transformed_system_prompt})

            prompt_messages.extend(full_conversations)
            ...
            transformed_ground_truth = transform_system_prompt(content, use_age_preference)
            ...
            extra_info = {
                ...
                "transformed_system_prompt": transformed_system_prompt,
                ...
                "use_age_preference": use_age_preference,
            }

            reward_model = {
                "style": "contact_stage_rule",
                "ground_truth": transformed_ground_truth,
...
def summarize_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    by_age_preference = Counter(row["extra_info"].get("use_age_preference") for row in rows)
    ...
    return {
        ...
        "age_preference_count": dict(by_age_preference),
        "coverage": {
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        default="/data/chengch/project/rl_remake/outputs/single_turn_rl_contact_stage_2k_v2",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Ratio of data to add age-based contact preference (default: 0.2 = 20%)",
    )
    ...
    for src in SOURCES:
        row_offset = sum(len(r) for r in rows_all)
        rows_all.extend(
    ...
    age_preference_count = int(len(rows) * args.age_preference_ratio)
    age_preference_indices = set(rng.sample(range(len(rows)), age_preference_count))

    print(f"[transform] Applying transformations to {len(age_preference_indices)} rows ({args.age_preference_ratio * 100:.1f}%)")

    for idx in age_preference_indices:
        row = rows[idx]

        system_prompt = row["extra_info"]["original_system_prompt"]
        transformed_system_prompt = transform_system_prompt(system_prompt, use_age_preference=True)
        row["extra_info"]["transformed_system_prompt"] = transformed_system_prompt

        transformed_ground_truth = transform_system_prompt(row["ground_truth"], use_age_preference=True)
        row["ground_truth"] = transformed_ground_truth
        row["reward_model"]["ground_truth"] = transformed_ground_truth
        row["extra_info"]["use_age_preference"] = True
    ...
    stats = {
        ...
        "targets": {
            ...
            "val_size": val_size,
            "strict_contact_signal": args.strict_contact_signal,
            "age_preference_ratio": args.age_preference_ratio,
        },
        "selected_counts": {
            "total": len(rows),
            ...
            "age_preference": age_preference_count,
        },
    ...
    print(f"[stats] age_preference rows: {age_preference_count} ({args.age_preference_ratio * 100:.1f}%)")
