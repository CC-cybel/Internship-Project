...
: "${CONTACT_TEACHER:=/data1/chengch/models/qwen3_8b_normal_mid_140}"
: "${MID_TEACHER:=/data1/chengch/models/qwen3_8b_normal_mid_140_250}"

: "${DATA_DIR:=/data/chengch/project/verl/recipe/opd_multi_teacher/data}"
: "${TRAIN_FILE:=${DATA_DIR}/opd_multi_teacher_10k.train.jsonl}"
: "${VAL_FILE:=${DATA_DIR}/opd_multi_teacher_10k.val.jsonl}"
: "${REBUILD_DATA:=false}"
...
: "${PROJECT_NAME:=verl_opd_multi_teacher}"
RUN_TAG="$(date +%Y%m%d_%H%M%S)"
: "${EXP_NAME:=qwen3_8b_multi_teacher_opd_${RUN_TAG}}"
: "${OUTPUT_BASE_DIR:=/data1/chengch/verl_outputs/opd_multi_teacher}"
...
if [[ "${REBUILD_DATA}" == "true" || ! -f "${TRAIN_FILE}" || ! -f "${VAL_FILE}" ]]; then
  "${PYTHON_BIN}" recipe/opd_multi_teacher/build_opd_multi_teacher_data.py \
    --output-dir "${DATA_DIR}" \
    --overwrite
fi
...
CMD=(
  ...
  "reward.custom_reward_function.path=recipe/opd_multi_teacher/reward_zero.py"
