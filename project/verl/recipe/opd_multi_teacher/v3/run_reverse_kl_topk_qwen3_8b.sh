...
: "${DATA_DIR:=/data/chengch/project/verl/recipe/opd_multi_teacher/v3/data}"
: "${TRAIN_FILE:=${DATA_DIR}/opd_multi_teacher_v3_teacher_generated.train.jsonl}"
: "${VAL_FILE:=${TRAIN_FILE}}"
: "${BUILD_DATA:=false}"
...
: "${PROJECT_NAME:=verl_opd_multi_teacher_v3}"
RUN_TAG="$(date +%Y%m%d_%H%M%S)"
: "${EXP_NAME:=qwen3_8b_v3_reverse_kl_topk_${RUN_TAG}}"
: "${OUTPUT_BASE_DIR:=/data1/chengch/verl_outputs/opd_multi_teacher/v3}"
...
if [[ "${BUILD_DATA}" == "true" || ! -f "${TRAIN_FILE}" ]]; then
  "${PYTHON_BIN}" recipe/opd_multi_teacher/v3/build_teacher_generated_data.py \
    --input "${SOURCE_FILE}" \
    --output-dir "${DATA_DIR}" \
...
CMD=(
  ...
  "actor_rollout_ref.rollout.agent.agent_loop_config_path=recipe/opd_multi_teacher/v3/teacher_forced_agent_loop.yaml"
  "actor_rollout_ref.rollout.agent.default_agent_loop=teacher_forced_agent"
  "actor_rollout_ref.ref.fsdp_config.param_offload=True"
  "actor_rollout_ref.ref.fsdp_config.model_dtype=bfloat16"
  "reward.reward_model.enable=False"
  "reward.reward_manager.name=naive"
  "reward.custom_reward_function.path=recipe/opd_multi_teacher/v3/reward_zero.py"
...
echo "[INFO] distill=reverse_kl_topk, topk=${DISTILL_TOPK}, teacher_forced_agent=true"
