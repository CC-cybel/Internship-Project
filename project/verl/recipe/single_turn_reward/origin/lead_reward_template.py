"""Template for lead reward collaborator."""
...
# Fill this prompt by lead-reward owner.
...
async def compute_lead_score(
    ...
    **kwargs: Any,
    ...
    if not LEAD_PROMPT_TEMPLATE.strip():
        return {"score": 0.0, "raw": "", "status": "lead_prompt_empty"}
    if not reward_router_address:
        return {"score": 0.0, "raw": "", "status": "reward_router_address_missing"}

    judge_model = kwargs.get("lead_judge_model") or kwargs.get("judge_model")
    if not judge_model:
    ...
    payload = {
        ...
        "temperature": float(kwargs.get("lead_judge_temperature", 0.0)),
    ...
    try:
        ...
        score = _parse_score(raw, score_max=float(kwargs.get("lead_judge_score_max", 10.0)))
