...
: "${RESUME_FROM_PATH:=/data1/chengch/verl_outputs/grpo_single_turn/qwen3_8b_4gpu_stage4_contact_rubric_reward_20260603_091851/global_step_450}"
...
: "${RUBRIC_INDEX_PATH:=/data/chengch/project/verl/recipe/single_turn_reward/v5/rubrics/rubric_index.json}"
: "${HARD_CONFIG_PATH:=/data/chengch/project/verl/recipe/single_turn_reward/v5/contact_reward_hard_config.json}"
: "${JUDGE_MAX_TOKENS:=1600}"

: "${PROJECT_NAME:=verl_grpo_single_turn}"
RUN_TAG="$(date +%Y%m%d_%H%M%S)"
: "${EXP_NAME:=qwen3_8b_4gpu_stage4_contact_rubric_reward_${RUN_TAG}}"
...
if [[ "${RESUME_MODE}" == "resume_path" ]]; then
  if [[ -z "${RESUME_FROM_PATH}" || ! -d "${RESUME_FROM_PATH}" ]]; then
    echo "[ERROR] resume_path mode requires an existing RESUME_FROM_PATH."
    echo "[ERROR] RESUME_FROM_PATH=${RESUME_FROM_PATH:-<empty>}"
    exit 1
  fi
  if [[ ! -d "${RESUME_FROM_PATH}/actor" ]]; then
    echo "[ERROR] Missing actor checkpoint directory: ${RESUME_FROM_PATH}/actor"
    exit 1
  fi
  if [[ ! -f "${RESUME_FROM_PATH}/data.pt" ]]; then
    echo "[ERROR] Missing dataloader state: ${RESUME_FROM_PATH}/data.pt"
    exit 1
  fi
  if [[ ! -f "${RESUME_FROM_PATH}/actor/fsdp_config.json" ]]; then
    echo "[ERROR] Missing actor fsdp_config.json in ${RESUME_FROM_PATH}/actor"
    exit 1
  fi
  for rank in 0 1 2 3; do
    for prefix in model optim extra_state; do
      shard="${RESUME_FROM_PATH}/actor/${prefix}_world_size_${N_GPUS_PER_NODE}_rank_${rank}.pt"
      if [[ ! -f "${shard}" ]]; then
        echo "[ERROR] Missing checkpoint shard: ${shard}"
        exit 1
      fi
    done
  done
...
CMD=(
  ...
  "reward.custom_reward_function.path=recipe/single_turn_reward/v5/reward_function_stage4_contact_rubric_cloud.py"
  ...
  "+reward.custom_reward_function.reward_kwargs.rubric_index_path=${RUBRIC_INDEX_PATH}"
  "+reward.custom_reward_function.reward_kwargs.hard_config_path=${HARD_CONFIG_PATH}"
  "+reward.custom_reward_function.reward_kwargs.judge_max_tokens=${JUDGE_MAX_TOKENS}"
...
echo "[INFO] using reward=recipe/single_turn_reward/v5/reward_function_stage4_contact_rubric_cloud.py"
echo "[INFO] rubric_index_path=${RUBRIC_INDEX_PATH}"
echo "[INFO] hard_config_path=${HARD_CONFIG_PATH}"
