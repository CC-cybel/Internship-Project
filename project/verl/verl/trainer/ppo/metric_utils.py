...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # GRPO group-wise reward dispersion metrics (grouped by uid).
    # This helps monitor within-group reward variance for each prompt group.
    if "uid" in batch.non_tensor_batch:
        uid_list = batch.non_tensor_batch["uid"]
        reward_np = sequence_reward.detach().float().cpu().numpy()
        non_aborted_np = non_aborted_mask.detach().cpu().numpy().astype(bool)

        uid_to_rewards: defaultdict[Any, list[float]] = defaultdict(list)
        for uid, r, ok in zip(uid_list, reward_np, non_aborted_np, strict=False):
            if ok:
        ...
        for rewards in uid_to_rewards.values():
            ...
            group_vars.append(float(np.var(arr)))
            group_stds.append(float(np.std(arr)))
            group_sizes.append(int(arr.size))

        if len(group_vars) > 0:
            metrics.update(
                {
                    "grpo/group_reward_var/mean": float(np.mean(group_vars)),
                    "grpo/group_reward_var/max": float(np.max(group_vars)),
                    "grpo/group_reward_var/min": float(np.min(group_vars)),
                    ...
                    "grpo/group_count": int(len(group_vars)),
                    "grpo/group_size/mean": float(np.mean(group_sizes)),
                    "grpo/group_size/max": int(np.max(group_sizes)),
                    "grpo/group_size/min": int(np.min(group_sizes)),
                }
            )
