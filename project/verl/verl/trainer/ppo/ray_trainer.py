...
class RayPPOTrainer:
    ...
    def _get_gen_batch(self, batch: DataProto) -> DataProto:
        reward_keys = set({"data_source", "reward_model", "extra_info", "uid", "teacher_route"}) & batch.non_tensor_batch.keys()
    ...
    @staticmethod
    def _drop_duplicate_non_tensor_keys(left: DataProto, right: DataProto) -> None:
        duplicate_keys = set(left.non_tensor_batch.keys()) & set(right.non_tensor_batch.keys())
        if duplicate_keys:
            right.pop(non_tensor_batch_keys=list(duplicate_keys))
    ...
    def _validate(self, merged: bool = False):
        ...
        for test_data in self.val_dataloader:
            ...
            self._drop_duplicate_non_tensor_keys(test_batch, test_output_gen_batch)
    ...
    def _load_checkpoint(self):
        if self.config.trainer.resume_mode in ("disable", "empty"):
            return 0
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        with marked_timer("gen_max", timing_raw, color="purple"):
                            ...
                            if curr_step_profile:
                                self.async_rollout_manager.stop_profile()
                            self._drop_duplicate_non_tensor_keys(batch, gen_baseline_output)
