...
class vLLMHttpServer:
    ...
    async def generate(
        ...
        # vLLM requires max_tokens >= 1. Some boundary prompts can consume the
        # configured prompt+response budget and lead to max_tokens == 0.
        # Keep the run alive by requesting at least one token when context allows.
        if max_tokens == 0 and max_possible_tokens > 0:
            logger.warning(
                "max_tokens computed as 0 (prompt_len=%s, prompt_cfg=%s, response_cfg=%s, max_model_len=%s); "
                "falling back to max_tokens=1",
                len(prompt_ids),
